...
def detect_key_species(json_elements, all_phylums, phylum_species=None):
    if phylum_species:
        unions = {phylum_species[species_union]: species_union
            		for species_union in json_elements["union_bacteria"]}
        intersections = {phylum_species[species_intersection]: species_intersection
    ...
    with open(key_species_stats_output, "w") as key_stats_file, open(
        ...
        for target_categorie in target_categories:
            ...
            statswriter.writerow([target_categorie, str(len_target[target_categorie]), str(len_min_sol[target_categorie]),
									str(len_union[target_categorie]), str(len_intersection[target_categorie]),
                    				str(len_solution[target_categorie])])
